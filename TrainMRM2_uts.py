...
from utils import set_mrm2_parser, readucr, SingleTSDataset
import heapq
...
if __name__ == '__main__':
    ...
    result = {'Name': [], 'val_loss': [], 'val_tcrm2_acc': [], 'val_ttrm2_acc': [],'val_tcrm2_acc_final': [], 'val_ttrm2_acc_final': []}
...
#         if x_test_num>x_train_num*2 and x_test_num>1024:
#             x_test_idxes_all = list(range(x_test_num))
#             x_test_idxes = random.sample(x_test_idxes_all,max(x_train_num,x_train_num*2))
#             x_test,y_test = x_test[x_test_idxes],y_test[x_test_idxes]
        ...
        lambda_gamma_config = json.load(open('./lambda_gamma_config/os_cnn_univar.json','r'))
        if train_config.model_name.lower() == 'fcn':
            model = FCNMRM2(input_features_d=1,n_feature_maps=128,nb_classes=nb_classes).to(device)
            lambda_gamma_config = json.load(open('./lambda_gamma_config/fcn_univar.json','r'))
        elif train_config.model_name.lower() =='fcn_lstm':
            model= FCNLSTMMRM2(1,128,nb_classes,8).to(device)
            lambda_gamma_config = json.load(open('./lambda_gamma_config/fcn_lstm_univar.json','r'))
        elif train_config.model_name.lower()=='resnet':
            model = ResNetMRM2(1,64,nb_classes).to(device)
            lambda_gamma_config = json.load(open('./lambda_gamma_config/resnet_univar.json','r'))
        elif train_config.model_name.lower()=='inceptiontime':
            model= InceptionTimeMRM2(1,nb_classes=nb_classes).to(device)
            lambda_gamma_config = json.load(open('./lambda_gamma_config/inceptiontime_univar.json','r'))
        else:
        ...
        train_config.lambda_loss,train_config.gamma_loss = lambda_gamma_config[fname]['lambda'],lambda_gamma_config[fname]['gamma']
        ...
        if len(train_config.pretrain_model_dir) != 0:
            ...
            if os.path.exists(pretrain_model_path):
                ...
                print('there not have the path' ,pretrain_model_path)
...
#                                                                verbose=True, min_lr=0.0001)
    ...
    #         keys =list( train_label2idx.keys())
        ...
        print('the lambda_loss and gamma_loss is: ',train_config.lambda_loss,train_config.gamma_loss )
        ...
        for epoch in range(max_epoch):
            ...
            for i, (xs, labels, is_train, ids) in (enumerate(test_dataloader, 0)):
                if xs.shape[0]<nb_classes*2:
                    continue
                optimizer.zero_grad()
                attn_label = None

                sub_x_train_ids = shuffle_choice(batch_size_train)
                sub_xs_train = torch.from_numpy(x_train[sub_x_train_ids]).unsqueeze(1).type(torch.float32)
                ...
                loss = sum(losses[is_train]) / (sum(is_train.long())+1e-6) + \
                       train_config.lambda_loss * sum(losses_ttrm2[is_train]) / (sum(is_train.long())+1e-6)
            ...
            alpha_angle = 3
            class_loss = train_config.gamma_loss * multi_criterion_mean(torch.matmul(class_embed, class_embed.t())/mold_class_embedx*alpha_angle,
            ...
            if epoch > 0.3 * max_epoch:
                if best_loss > running_loss+ttrm2_running_loss:
                    ...
                    final_res = [running_loss+ttrm2_running_loss, acc_val, acc_ttrm2_val]
        ...
        x_train_pred = []
        model.eval()
        for start in range(0,x_train_num,128):
            end = start+128
            in_tensor = torch.from_numpy(x_train[start:end]).unsqueeze(1).type(torch.float32).to(device)
        ...
        for start in range(0,x_test.shape[0],128):
            ...
            in_tensor = torch.from_numpy(x_test[start:end]).unsqueeze(1).type(torch.float32).to(device)
            out_all = model(in_tensor,None)
