...
from utils import set_mrm2_parser, MultiTSDataset, readuea
import heapq
from Config import multiflist
...
if __name__ == '__main__':
    ...
    for fname in (flist):
        ...
        lambda_gamma_config = json.load(open('./lambda_gamma_config/os_cnn_multi.json','r'))
        if train_config.model_name.lower() == 'fcn':
            ...
            lambda_gamma_config = json.load(open('./lambda_gamma_config/fcn_multi.json','r'))
        elif train_config.model_name.lower() =='fcn_lstm':
            model= FCNLSTMMRM2(x_channel,128,nb_classes,8).to(device)
            lambda_gamma_config = json.load(open('./lambda_gamma_config/fcn_lstm_multi.json','r'))
        elif train_config.model_name.lower()=='resnet':
            model = ResNetMRM2(x_channel,64,nb_classes).to(device)
            lambda_gamma_config = json.load(open('./lambda_gamma_config/resnet_multi.json','r'))
        elif train_config.model_name.lower()=='inceptiontime':
            model= InceptionTimeMRM2(x_channel,nb_classes=nb_classes).to(device)
            lambda_gamma_config = json.load(open('./lambda_gamma_config/inceptiontime_multi.json','r'))
        else:
        ...
        result['val_loss'].append(float(final_res[0].cpu().detach().numpy()))
