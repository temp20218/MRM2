...
from utils import set_parser, readucr,MixedTSDataset,readuea,MultiTSDataset
...
if __name__ == '__main__':
    ...
    for fid, fname in enumerate(flist):
        ...
        train_dataset = MultiTSDataset(x_train, y_train,is_train=True)
        ...
        if train_config.model_name.lower() == 'fcn':
            model = FCNTS(input_features_d=int(x_train.shape[1]),n_feature_maps=128,nb_classes=nb_classes).to(device)
        elif train_config.model_name.lower() =='fcn_lstm':
            model= FCNLSTMTS(int(x_train.shape[1]),128,nb_classes,8).to(device)
        elif train_config.model_name.lower()=='resnet':
            model = ResNetTS(int(x_train.shape[1]),64,nb_classes).to(device)
        elif train_config.model_name.lower()=='inceptiontime':
            model= InceptionTime(int(x_train.shape[1]),nb_classes=nb_classes).to(device)
        else:
